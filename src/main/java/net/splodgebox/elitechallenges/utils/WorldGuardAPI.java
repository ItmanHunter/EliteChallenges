    ...
    public WorldGuardAPI(Plugin plugin, Plugin owningPlugin) {
        ...
        if (plugin instanceof WorldGuardPlugin) {
            ...
            try {
                ...
                Chat.log("&aWorldGuard Support Success");
            } catch (Exception ex) {
                Chat.log("&aWorldGuard Support Success");
            }
        }
        ...
        if (!initialized) {
            ...
            if (worldGuard != null) {
                try {
                    ...
                    breakFlag = (StateFlag)flagsClass.getField("BLOCK_BREAK").get(null);
                } catch (Exception ex) {
    ...
    public boolean canBreak(Player player, Block block) {
        ...
        if (block != null && createQueryMethod != null && regionContainer != null) {
            try {
                ...
                if (locationAdaptMethod != null) {
                    Object location = locationAdaptMethod.invoke(null, block.getLocation());
                    result = (boolean)regionQueryTestStateMethod.invoke(query, location, getAssociable(player), new StateFlag[]{breakFlag});
                } else {
                    result = (boolean)regionQueryTestStateMethod.invoke(query, block.getLocation(), getAssociable(player), new StateFlag[]{breakFlag});
                }
                return result;
            } catch (Exception ex) {
                owningPlugin.getLogger().log(Level.WARNING, "An error occurred querying WorldGuard", ex);
